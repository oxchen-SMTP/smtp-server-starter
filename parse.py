...
def main():
    global stream, next_char, state, forward_paths
    for line in sys.stdin:
        ...
        """
        Requirements:
            - Save forward-path -> access and write to forward/forward-path
            - Save data body -> write to intended forward paths
        """
        ...
        if state == 3:
            ...
            print("Data body recognized, not yet implemented")
        else:
            res = recognize_cmd()
            command = res[0]
            states = res[1]
...
def read_data() -> str:
    global stream, next_char
    data = ""
    buffer = ""  # we don't want to attach <CRLF>.<CRLF> to
    while buffer != "\n.\n":
